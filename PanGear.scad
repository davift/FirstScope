	...
	if(which == 1)
	{
        ...
        union(){
            difference(){
                ...
                //DIFFERENCE:
                //shafthole
                translate([0,0,-TT]) cylinder(r=gear1_shaft_r, h=gear_h++ST, $fn=90);
            }
        }
        translate([2.5,0,5]) cube([2,5,10], center=true);
        translate([-2.5,0,5]) cube([2,5,10], center=true);
    }
	else
	{
		...
		difference(){
			...
			//DIFFERENCE:
			//shafthole
			translate([0,0,-TT-1]) cylinder(r=gear2_shaft_r, h=gear_h+gear_shaft_h+ST, $fn=90);
            cylinder(d=201, h=13, $fn=90);
            cylinder(d=24, h=100, $fn=90);
		}
	}
}
...
{
	mirror([0,0,1]) translate([-g1p_r,0,-gear_h]) rotate([0,0,($t*360/gear1_teeth)]) gearsbyteethanddistance(t1 = gear1_teeth, t2=gear2_teeth, d=distance_between_axels, which=1);
